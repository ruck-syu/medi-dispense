...
class BackgroundDoseScheduler {
  ...
  Future<void> scheduleAllPendingDoseAlarms() async {
    ...
    final pending = await DatabaseHelper.instance.getPendingDoseRecordsFromNow();
  ...
  Future<void> _scheduleDoseAlarm(DoseRecord dose) async {
    final trigger = DateTime.tryParse('${dose.scheduledDate} ${dose.scheduledTime}:00');
  ...
  int _alarmIdForDose(DoseRecord dose) {
    final key = '${dose.medicineId}-${dose.scheduledDate}-${dose.scheduledTime}';
...
Future<void> backgroundDoseAlarmCallback() async {
  ...
  for (final dose in dueDoses) {
    final medicine = await DatabaseHelper.instance.getMedicineById(dose.medicineId);
